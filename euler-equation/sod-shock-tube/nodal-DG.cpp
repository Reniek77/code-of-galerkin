...
Vector True_Solution(const double x,const double t){
    Solution(0)=1.0+(0.5*sin((x-t))*sin((x-t)));Solution(1)=1.0;Solution(2)=2.0;
    return Solution;
}
...
Vector Entropy_Conservative_Flux(const State_of_Fluid UL,const State_of_Fluid UR){
    ...
    double aL=sqrt(UL.get_density()/UL.get_pressure());
    ZL(0)=aL;ZL(1)=aL*UL.get_velocity();ZL(2)=aL*UL.get_pressure();

    double aR=sqrt(UR.get_density()/UR.get_pressure());
    ZR(0)=aR;ZR(1)=aR*UR.get_velocity();ZR(2)=aR*UR.get_pressure();

    for(int i=0;i<3;++i)Z_ave(i)=0.5*ZL(i)+0.5*ZR(i);

    for(int i=0;i<3;++i)Z_ave_log(i)=Average_Under_Log(ZR(i),ZL(i));

    EC_Flux(0)=Z_ave(1)*Z_ave_log(2);
    EC_Flux(1)=Z_ave(2)/Z_ave(0)+Z_ave(1)/Z_ave(0)*EC_Flux(0);
    EC_Flux(2)=0.5*Z_ave(1)/Z_ave(0)*((gamma+1.0)/(gamma-1.0)*Z_ave_log(2)/Z_ave_log(0)+EC_Flux(1));
...
inline void init(){
    ...
    for(int i=1;i<=N;++i)
    {
        double left=-0.5+(i-1)*h;
        double right=-0.5+i*h;
...
void Final_Check(){
    ...
    for(register int i=1;i<=N;++i)
    {
        double tmp=0.0;
        for(register int j=0;j<=k;++j)
        tmp+=u_h[i](j).get_velocity()*GL.get_GL()[j].second;
    ...
    for(register int i=1;i<=N;++i)
    {
        double tmp=0.0;
        for(register int j=0;j<=k;++j)
        tmp+=u_h[i](j).get_pressure()*GL.get_GL()[j].second;
